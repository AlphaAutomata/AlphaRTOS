...
.equ TIMER0_BASE       , 0x40030000
.equ TIMER_TIMA_TIMEOUT, 0x00000001
.equ TIMER_O_ICR       , 0x00000024
...
isrContAddr:
	...
	.global runTask
	.type runTask, "function"
	.size runTask, .runTask_end-runTask
runTask:
...
.runTask_end:
	...
	.size switchContext, .switchContext_end-switchContext
switchContext:
	STMIA	R1, {R0-R12}			// save registers 0-12 to *oldframe
...
// The SysTick interrupt serves as the operating system tick timer for scheduling, time keeping, and
// task preemption. When the global currTaskID variable is set correctly, this interrupt service
// routine always guarentees that the kernel will run at least once every millisecond. This ISR
// does so by loading an IRQ return address onto the IRQ stack to drop out of the IRQ into the
// preempt subroutine. The preempt subroutine then performs the switchContext() function call as if
// it were the preempted task.
	.global SysTick_Handler
	.type SysTick_Handler, "function"
	.size SysTick_Handler, .SysTick_Handler_end-SysTick_Handler
SysTick_Handler:
	// increment uptime
	LDR		R0, =uptime
	LDR		R1, [R0]
...
preempt:
	PUSH	{R0-R3,R12,LR}			// push caller-save registers
	...
	MLA		R3, R1, R2, R0			// multiply-accumulate to index into taskTable to currTask
	
	// currTask.status = TASK_STATUS_PREEMPTED#
	MOV		R2, #0x00000010			// set R2 to TASK_STATUS_PREEMPTED
	STR		R2, [R3,#taskStatus]	// store task status
	
	// switchContext(&kframe, &(currTask.frame))#
	ADD		R1, R3, #taskFrame		// offset into the frame field of the current task entry,
...
// globals. Therefore we simply jump into the SysTick ISR right after global variable update
	...
	.type TIMER0A_Handler, "function"
	.size TIMER0A_Handler, .TIMER0A_Handler_end-TIMER0A_Handler
TIMER0A_Handler:
